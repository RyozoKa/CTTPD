...
{
	FORCEINLINE__ functor_storage() = default;
	FORCEINLINE__ functor_storage(const Functor & functor)
        : Functor(functor)
    {
    }
    template<typename... Args>
	FORCEINLINE__ Result operator()(Args &&... args)
    {
        return static_cast<Functor &>(*this)(std::forward<Args>(args)...);
    }
    template<typename... Args>
	FORCEINLINE__ Result operator()(Args &&... args) const
    {
...
struct functor_storage<Result, Result (*)(Args...)>
{
	...
	FORCEINLINE__ functor_storage(function_ptr function)
        : function(function)
    {
    }
	FORCEINLINE__ Result operator()(Args... args) const
	...
	FORCEINLINE__ operator function_ptr &()
    {
        return function;
    }
	FORCEINLINE__ operator const function_ptr &()
    {
        return function;
    }
};
...
struct KeyOrValueHasher : functor_storage<size_t, hasher>
{
	...
	FORCEINLINE__ KeyOrValueHasher() = default;
	FORCEINLINE__ KeyOrValueHasher(const hasher & hash)
        : hasher_storage(hash)
    {
    }
	FORCEINLINE__ size_t operator()(const key_type & key)
    {
        return static_cast<hasher_storage &>(*this)(key);
    }
	FORCEINLINE__ size_t operator()(const key_type & key) const
    {
        return static_cast<const hasher_storage &>(*this)(key);
    }
	FORCEINLINE__ size_t operator()(const value_type & value)
    {
        return static_cast<hasher_storage &>(*this)(value.first);
    }
	FORCEINLINE__ size_t operator()(const value_type & value) const
    {
        return static_cast<const hasher_storage &>(*this)(value.first);
    }
    template<typename F, typename S>
	FORCEINLINE__ size_t operator()(const std::pair<F, S> & value)
    {
        return static_cast<hasher_storage &>(*this)(value.first);
    }
    template<typename F, typename S>
	FORCEINLINE__ size_t operator()(const std::pair<F, S> & value) const
    {
        return static_cast<const hasher_storage &>(*this)(value.first);
    }
};
...
struct KeyOrValueEquality : functor_storage<bool, key_equal>
{
	...
	FORCEINLINE__ KeyOrValueEquality() = default;
	FORCEINLINE__ KeyOrValueEquality(const key_equal & equality)
        : equality_storage(equality)
    {
    }
	FORCEINLINE__ bool operator()(const key_type & lhs, const key_type & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs, rhs);
    }
	FORCEINLINE__ bool operator()(const key_type & lhs, const value_type & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs, rhs.first);
    }
	FORCEINLINE__ bool operator()(const value_type & lhs, const key_type & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs.first, rhs);
    }
	FORCEINLINE__ bool operator()(const value_type & lhs, const value_type & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs.first, rhs.first);
    }
    template<typename F, typename S>
	FORCEINLINE__ bool operator()(const key_type & lhs, const std::pair<F, S> & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs, rhs.first);
    }
    template<typename F, typename S>
	FORCEINLINE__ bool operator()(const std::pair<F, S> & lhs, const key_type & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs.first, rhs);
    }
    template<typename F, typename S>
	FORCEINLINE__ bool operator()(const value_type & lhs, const std::pair<F, S> & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs.first, rhs.first);
    }
    template<typename F, typename S>
	FORCEINLINE__ bool operator()(const std::pair<F, S> & lhs, const value_type & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs.first, rhs.first);
    }
    template<typename FL, typename SL, typename FR, typename SR>
	FORCEINLINE__ bool operator()(const std::pair<FL, SL> & lhs, const std::pair<FR, SR> & rhs)
    {
        return static_cast<equality_storage &>(*this)(lhs.first, rhs.first);
    }
};
...
{
	FORCEINLINE__ static constexpr sherwood_v3_entry special_end_entry()
    {
	...
	FORCEINLINE__ bool has_value() const
    {
        return distance_from_desired >= 0;
    }
	FORCEINLINE__ bool is_empty() const
    {
        return distance_from_desired < 0;
    }
	FORCEINLINE__ bool is_at_desired_position() const
    {
        return distance_from_desired <= 0;
    }
    template<typename... Args>
	FORCEINLINE__ void emplace(int8_t distance, Args &&... args)
    {
        new (std::addressof(value)) T(std::forward<Args>(args)...);
        distance_from_desired = distance;
    }

	FORCEINLINE__ void destroy_value()
    {
        value.~T();
...
{
	FORCEINLINE__ static constexpr sherwood_v3_entry_constexpr special_end_entry()
    {
...
MAINFRAME_API int8_t log2(size_t value);
FORCEINLINE__ void throw_out_of_range();

template<typename T, bool>
struct AssignIfTrue
{
	FORCEINLINE__ void operator()(T & lhs, const T & rhs)
    {
        lhs = rhs;
    }
	FORCEINLINE__ void operator()(T & lhs, T && rhs)
    {
        lhs = std::move(rhs);
    }
};
...
{
	FORCEINLINE__ void operator()(T &, const T &)
    {
    }
	FORCEINLINE__ void operator()(T &, T &&)
    {
    }
};

MAINFRAME_API size_t next_power_of_two(size_t i);
...
public:
	...
	FORCEINLINE__ sherwood_v3_table()
    {
    }
	FORCEINLINE__  explicit sherwood_v3_table(size_type bucket_count, const ArgumentHash & hash = ArgumentHash(), const ArgumentEqual & equal = ArgumentEqual(), const ArgumentAlloc & alloc = ArgumentAlloc())
        : EntryAlloc(alloc), Hasher(hash), Equal(equal)
    {
        rehash(bucket_count);
    }
	FORCEINLINE__ sherwood_v3_table(size_type bucket_count, const ArgumentAlloc & alloc)
        : sherwood_v3_table(bucket_count, ArgumentHash(), ArgumentEqual(), alloc)
    {
    }
	FORCEINLINE__ sherwood_v3_table(size_type bucket_count, const ArgumentHash & hash, const ArgumentAlloc & alloc)
        : sherwood_v3_table(bucket_count, hash, ArgumentEqual(), alloc)
    {
    }
	FORCEINLINE__ explicit sherwood_v3_table(const ArgumentAlloc & alloc)
        : EntryAlloc(alloc)
    {
    }
    template<typename It>
	FORCEINLINE__ sherwood_v3_table(It first, It last, size_type bucket_count = 0, const ArgumentHash & hash = ArgumentHash(), const ArgumentEqual & equal = ArgumentEqual(), const ArgumentAlloc & alloc = ArgumentAlloc())
        : sherwood_v3_table(bucket_count, hash, equal, alloc)
    {
        insert(first, last);
    }
    template<typename It>
	FORCEINLINE__ sherwood_v3_table(It first, It last, size_type bucket_count, const ArgumentAlloc & alloc)
        : sherwood_v3_table(first, last, bucket_count, ArgumentHash(), ArgumentEqual(), alloc)
    {
    }
    template<typename It>
	FORCEINLINE__ sherwood_v3_table(It first, It last, size_type bucket_count, const ArgumentHash & hash, const ArgumentAlloc & alloc)
        : sherwood_v3_table(first, last, bucket_count, hash, ArgumentEqual(), alloc)
    {
    }
	FORCEINLINE__ sherwood_v3_table(std::initializer_list<T> il, size_type bucket_count = 0, const ArgumentHash & hash = ArgumentHash(), const ArgumentEqual & equal = ArgumentEqual(), const ArgumentAlloc & alloc = ArgumentAlloc())
        : sherwood_v3_table(bucket_count, hash, equal, alloc)
    {
        if (bucket_count == 0)
            rehash(il.size());
	...
	FORCEINLINE__ sherwood_v3_table(std::initializer_list<T> il, size_type bucket_count, const ArgumentAlloc & alloc)
        : sherwood_v3_table(il, bucket_count, ArgumentHash(), ArgumentEqual(), alloc)
    {
    }
	FORCEINLINE__ sherwood_v3_table(std::initializer_list<T> il, size_type bucket_count, const ArgumentHash & hash, const ArgumentAlloc & alloc)
        : sherwood_v3_table(il, bucket_count, hash, ArgumentEqual(), alloc)
    {
    }
	FORCEINLINE__ sherwood_v3_table(const sherwood_v3_table & other)
        : sherwood_v3_table(other, AllocatorTraits::select_on_container_copy_construction(other.get_allocator()))
    {
    }
	FORCEINLINE__ sherwood_v3_table(const sherwood_v3_table & other, const ArgumentAlloc & alloc)
	...
	FORCEINLINE__ sherwood_v3_table(sherwood_v3_table && other) noexcept
        : EntryAlloc(std::move(other)), Hasher(std::move(other)), Equal(std::move(other))
    {
        swap_pointers(other);
    }
	FORCEINLINE__ sherwood_v3_table(sherwood_v3_table && other, const ArgumentAlloc & alloc) noexcept
        : EntryAlloc(alloc), Hasher(std::move(other)), Equal(std::move(other))
    {
        swap_pointers(other);
    }
	FORCEINLINE__ sherwood_v3_table & operator=(const sherwood_v3_table & other)
    {
	...
	FORCEINLINE__ sherwood_v3_table & operator=(sherwood_v3_table && other) noexcept
    {
        if (this == std::addressof(other))
            return *this;
	...
	FORCEINLINE__ const allocator_type & get_allocator() const
    {
        return static_cast<const allocator_type &>(*this);
    }
	FORCEINLINE__ const ArgumentEqual & key_eq() const
    {
        return static_cast<const ArgumentEqual &>(*this);
    }
	FORCEINLINE__ const ArgumentHash & hash_function() const
    {
    ...
    struct templated_iterator
    {
		...
		FORCEINLINE__ friend bool operator==(const templated_iterator & lhs, const templated_iterator & rhs)
        {
		...
		FORCEINLINE__ templated_iterator & operator++()
        {
            do
            {
                ++current;
		...
		FORCEINLINE__ templated_iterator operator++(int)
        {
		...
		FORCEINLINE__ ValueType & operator*() const
        {
		...
		FORCEINLINE__ operator templated_iterator<const value_type>() const
        {
            return { current };
        }
    };
	...
	FORCEINLINE__ const_iterator begin() const
    {
        for (EntryPointer it = entries;; ++it)
        {
            if (it->has_value())
	...
	FORCEINLINE__ const_iterator cbegin() const
    {
        return begin();
    }
	FORCEINLINE__ iterator end()
    {
        return { entries + static_cast<ptrdiff_t>(num_slots_minus_one + max_lookups) };
    }
	FORCEINLINE__ const_iterator end() const
    {
        return { entries + static_cast<ptrdiff_t>(num_slots_minus_one + max_lookups) };
    }
	FORCEINLINE__ const_iterator cend() const
    {
        return end();
    }

	FORCEINLINE__ iterator find(const FindKey & key)
    {
	...
	FORCEINLINE__ const_iterator find(const FindKey & key) const
    {
        return const_cast<sherwood_v3_table *>(this)->find(key);
    }
	FORCEINLINE__ size_t count(const FindKey & key) const
    {
        return find(key) == end() ? 0 : 1;
    }
	FORCEINLINE__ std::pair<iterator, iterator> equal_range(const FindKey & key)
    {
	...
	FORCEINLINE__ std::pair<const_iterator, const_iterator> equal_range(const FindKey & key) const
    {
	...
	FORCEINLINE__ std::pair<iterator, bool> emplace(Key && key, Args &&... args)
    {
        size_t index = hash_policy.index_for_hash(hash_object(key), num_slots_minus_one);
	...
	FORCEINLINE__ std::pair<iterator, bool> insert(const value_type & value)
    {
        return emplace(value);
    }
	FORCEINLINE__ std::pair<iterator, bool> insert(value_type && value)
    {
        return emplace(std::move(value));
    }
    template<typename... Args>
	FORCEINLINE__ iterator emplace_hint(const_iterator, Args &&... args)
    {
        return emplace(std::forward<Args>(args)...).first;
    }
	FORCEINLINE__ iterator insert(const_iterator, const value_type & value)
    {
        return emplace(value).first;
    }
	FORCEINLINE__ iterator insert(const_iterator, value_type && value)
    {
        return emplace(std::move(value)).first;
    }

    template<typename It>
	FORCEINLINE__ void insert(It begin, It end)
    {
        for (; begin != end; ++begin)
        {
            emplace(*begin);
	...
	FORCEINLINE__ void insert(std::initializer_list<value_type> il)
    {
        insert(il.begin(), il.end());
    }

	FORCEINLINE__ void rehash(size_t num_buckets)
    {
        num_buckets = max(num_buckets, static_cast<size_t>(std::ceil(num_elements / static_cast<double>(_max_load_factor))));
        if (num_buckets == 0)
        {
            reset_to_empty_state();
            return;
	...
	FORCEINLINE__ void reserve(size_t num_elements)
    {
	...
	FORCEINLINE__ convertible_to_iterator erase(const_iterator to_erase)
    {
	...
	FORCEINLINE__ iterator erase(const_iterator begin_it, const_iterator end_it)
    {
        ...
        if (end_it == this->end())
            return this->end();
        ptrdiff_t num_to_move = min(static_cast<ptrdiff_t>(end_it.current->distance_from_desired), end_it.current - begin_it.current);
        ...
        for (EntryPointer it = end_it.current; !it->is_at_desired_position();)
        {
            ...
            it->destroy_value();
            ++it;
            num_to_move = min(static_cast<ptrdiff_t>(it->distance_from_desired), num_to_move);
        }
        return { to_return };
    }

	FORCEINLINE__ size_t erase(const FindKey & key)
    {
        auto found = find(key);
        if (found == end())
            return 0;
	...
	FORCEINLINE__ void shrink_to_fit()
    {
        rehash_for_other_container(*this);
    }

	FORCEINLINE__ void swap(sherwood_v3_table & other)
    {
        using std::swap;
        swap_pointers(other);
	...
	FORCEINLINE__ size_t size() const
    {
        return num_elements;
    }
	FORCEINLINE__ size_t max_size() const
    {
        return (AllocatorTraits::max_size(*this)) / sizeof(Entry);
    }
	FORCEINLINE__ size_t bucket_count() const
    {
        return num_slots_minus_one + 1;
    }
	FORCEINLINE__ size_type max_bucket_count() const
    {
        return (AllocatorTraits::max_size(*this) - min_lookups) / sizeof(Entry);
    }
	FORCEINLINE__ size_t bucket(const FindKey & key) const
    {
        return hash_policy.index_for_hash(hash_object(key), num_slots_minus_one);
    }
	FORCEINLINE__ float load_factor() const
    {
	...
	FORCEINLINE__ void max_load_factor(float value)
    {
        _max_load_factor = value;
    }
	FORCEINLINE__ float max_load_factor() const
    {
        return _max_load_factor;
    }

	FORCEINLINE__ bool empty() const
    {
...
private:
	...
	FORCEINLINE__ static int8_t compute_max_lookups(size_t num_buckets)
    {
        int8_t desired = detailv3::log2(num_buckets);
        return max(detailv3::min_lookups, desired);
    }

	FORCEINLINE__ size_t num_buckets_for_reserve(size_t num_elements) const
    {
        return static_cast<size_t>(std::ceil(num_elements / min(0.5, static_cast<double>(_max_load_factor))));
    }
	FORCEINLINE__ void rehash_for_other_container(const sherwood_v3_table & other)
    {
        rehash(min(num_buckets_for_reserve(other.size()), other.bucket_count()));
    }

	FORCEINLINE__ void swap_pointers(sherwood_v3_table & other)
    {
        using std::swap;
	...
	FORCEINLINE__ void grow()
    {
        rehash(max(size_t(4), 2 * bucket_count()));
    }

	FORCEINLINE__ void deallocate_data(EntryPointer begin, size_t num_slots_minus_one, int8_t max_lookups)
    {
	...
	FORCEINLINE__ void reset_to_empty_state()
    {
        deallocate_data(entries, num_slots_minus_one, max_lookups);
    ...
    template<typename U>
	FORCEINLINE__ size_t hash_object(const U & key)
    {
        return static_cast<Hasher &>(*this)(key);
    }
    template<typename U>
	FORCEINLINE__ size_t hash_object(const U & key) const
    {
        return static_cast<const Hasher &>(*this)(key);
    }
    template<typename L, typename R>
	FORCEINLINE__ bool compares_equal(const L & lhs, const R & rhs)
    {
    ...
    struct convertible_to_iterator
    {
		...
		FORCEINLINE__ operator iterator()
        {
            if (it->has_value())
                return { it };
            else
		...
		FORCEINLINE__ operator const_iterator()
        {
            if (it->has_value())
                return { it };
            else
...
struct prime_number_hash_policy
{
	...
	FORCEINLINE__ void commit(uint8_t new_prime_index)
    {
        prime_index = new_prime_index;
    }
	FORCEINLINE__ void reset()
    {
        prime_index = 0;
    }
...
struct power_of_two_hash_policy
{
	FORCEINLINE__ size_t index_for_hash(size_t hash, size_t num_slots_minus_one) const
    {
        return hash & num_slots_minus_one;
    }
	FORCEINLINE__ int8_t next_size_over(size_t & size) const
    {
        size = detailv3::next_power_of_two(size);
        return 0;
    }
	FORCEINLINE__ void commit(int8_t)
    {
    }
	FORCEINLINE__ void reset()
    {
...
public:
    ...
    using Table::Table;
	FORCEINLINE__ flat_hash_map()
    {
    }

	FORCEINLINE__ flat_hash_map(size_t _reserve)
	{
		reserve(_reserve);
	}

	FORCEINLINE__ V & operator[](const K & key)
    {
        return emplace(key, convertible_to_value()).first->second;
    }
	FORCEINLINE__ V & operator[](K && key)
    {
        return emplace(std::move(key), convertible_to_value()).first->second;
    }
	FORCEINLINE__ V & at(const K & key)
    {
	...
	FORCEINLINE__ const V & at(const K & key) const
    {
        auto found = this->find(key);
        if (found == this->end())
            throw std::out_of_range("Argument passed to at() was not in the map.");
    ...
    using Table::emplace;
	FORCEINLINE__ std::pair<typename Table::iterator, bool> emplace()
    {
        return emplace(key_type(), convertible_to_value());
    }

	FORCEINLINE__ friend bool operator==(const flat_hash_map & lhs, const flat_hash_map & rhs)
    {
...
private:
    ...
    {
		FORCEINLINE__ operator V() const
        {
            return V();
        }
    };
...
public:
    ...
    using Table::Table;
	FORCEINLINE__ flat_hash_set()
    {
    }

    template<typename... Args>
	FORCEINLINE__ std::pair<typename Table::iterator, bool> emplace(Args &&... args)
    {
        return Table::emplace(T(std::forward<Args>(args)...));
    }
	FORCEINLINE__ std::pair<typename Table::iterator, bool> emplace(const key_type & arg)
    {
        return Table::emplace(arg);
    }
	FORCEINLINE__ std::pair<typename Table::iterator, bool> emplace(key_type & arg)
    {
        return Table::emplace(arg);
    }
	FORCEINLINE__ std::pair<typename Table::iterator, bool> emplace(const key_type && arg)
	...
	FORCEINLINE__ friend bool operator==(const flat_hash_set & lhs, const flat_hash_set & rhs)
    {
        if (lhs.size() != rhs.size())
            return false;
